...
function renderExercise(){ const s=store.getState(), a=s.activeSession; if(!a){setScreen(SCREEN_IDS.HOME);return;} const ex=getExerciseById(a.currentExerciseId); if(!ex){setScreen(SCREEN_IDS.HOME);return;} const pv=getExercisePhaseValue(ex.id,a.phaseId); const totalSets=a.plannedSetsByExercise[ex.id]; let workTimer=null; let setCompletionHandled=false;
 ...
 if(workTimer){let displayedRemaining=workTimer.remainingSeconds;activeInterval=setInterval(()=>{const remaining=Math.max(0,Math.ceil((workEndsAt-Date.now())/1000));if(remaining!==displayedRemaining){displayedRemaining=remaining;view.updateWorkTimer({remainingSeconds:remaining,finished:remaining===0});}if(remaining===0){clearInterval(activeInterval);activeInterval=null;}},250);}
}
