...
let currentView=null; let activeInterval=null; let workTimerKey=null; let workEndsAt=null; let restFeedbackKey=null;
...
function renderExercise(){ const s=store.getState(), a=s.activeSession; if(!a){setScreen(SCREEN_IDS.HOME);return;} const ex=getExerciseById(a.currentExerciseId); if(!ex){setScreen(SCREEN_IDS.HOME);return;} const pv=getExercisePhaseValue(ex.id,a.phaseId); const totalSets=a.plannedSetsByExercise[ex.id]; let workTimer=null;
